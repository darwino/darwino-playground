...
var doc1 = store.newDocument(); doc1.save(darwino.jstore.Document.SAVE_NOREAD);
s += "Created a new document with no read mark, read="+doc1.isRead()+"\n";

// Load the document -> should be marked as read now
doc.markRead(false);
var doc2 = store.loadDocument(doc.getUnid(),darwino.jstore.Store.DOCUMENT_NOREADMARK);
s += "Read the document with no read mark, read="+doc.isRead()+"\n";


// Load the document -> should be marked as read now
var doc2 = store.loadDocument(doc.getUnid());
